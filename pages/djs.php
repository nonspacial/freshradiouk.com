...
<div id="profilesPage" class="container">
    <div id="djList" class="col-md-3">
		...
		<?php	
			$djlist=$dbc->query("SELECT * FROM dj_profiles ORDER BY dj_name");
            #Get Page Content Query
            while($list=mysqli_fetch_assoc($djlist)) {?>
    ...
    <div id="djProfile" class="col-md-9 panel panel-default" style="background:none;">
       	<?php	$opened=$dbc->query("SELECT * FROM dj_profiles WHERE id=$_GET[id]");
				$openedlist=mysqli_fetch_assoc($opened);
			 	if ($openedlist) { ?>
                        <div id="bio_<?php echo $openedlist['id'];?>" class="col-md-12 bio" style="height:700px;color:#DCDCDC;"><h1><?php echo $openedlist['dj_name'];?></h1><?php echo html_entity_decode($openedlist['bio'])?></div>					
		<?php		}else{?>
