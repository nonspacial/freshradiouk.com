    ...
    <div id="djProfile" class="col-md-9 panel panel-default" style="background:none;">
       	<?php	$opened=$dbc->query("SELECT * FROM dj_profiles WHERE id=$_GET[id]");
				$list=mysqli_fetch_assoc($opened);
			 	if ($list) { ?>
                        <div id="bio_<?php echo $list['id'];?>" class="col-md-12 bio" style="height:700px"><?php echo html_entity_decode($list['bio'])?></div>					
		<?php		}else{?>
                        <div id="bioMain" class="col-md-12 bio" style="height:700px"><h1>SOME WELCOME TO PROFILES TEXT MF!!!!!!!!!!!</h1></div>					
		<?php 		}?>
    </div>
</div>
